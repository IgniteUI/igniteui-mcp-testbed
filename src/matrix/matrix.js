...
const { WORK, ARTIFACT_DIR } = require('../config');
const { killTree } = require('../proc/exec');
const { killWatcher } = require('../proc/watcher');
const { createSSE } = require('../stream/sse');
const { cleanupAppDir } = require('./cleanup');
const { entryDirName, newMatrixId } = require('./variants');
...
const sse = createSSE();
const broadcast = (obj) => sse.broadcast(obj);
...
function begin(combos, { prompt, fixed }) {
  ...
  runMatrix(combos, { prompt, matrixId, fixed }).catch((e) => {
