...
const { FRAMEWORKS, APP_PORT, subst } = require('../frameworks');
const { translate } = require('../mcp-translate');
const { discoverRoutes } = require('../capture/route-discovery');
const { shoot } = require('../capture/screenshots');
const { parseOpencodeStats } = require('../capture/usage');
const {
  APP_DIR, LOG_DIR, OPENCODE_PORT, AGENT_TIMEOUT_MS, APP_READY_TIMEOUT_MS, MCP_COMMAND_BY_CLASS,
} = require('../config');
const { run, capture } = require('../proc/exec');
const { spawnWatcher, killWatcher } = require('../proc/watcher');
const { waitForPort, waitForPortFree, waitForAppReady } = require('../proc/ports');
const { ensureDirs, sleep, rmrf } = require('../proc/fsutil');
const { writeOpencodeConfig, providerEnvFor, writePrepareFile } = require('./opencode-config');
const { classify } = require('./mcp-classify');
const { pruneSkills } = require('./skills');
const { cleanupAppDir } = require('../matrix/cleanup');
